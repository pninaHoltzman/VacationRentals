...
import { getAllCategories, getApartmentById, getCity, updateApartment } from '../api';
import { useParams, useNavigate } from 'react-router';
import Swal from 'sweetalert2';

export const Edit = () => {
  const { it } = useParams(); // קבלת ה-ID של הדירה
  ...
  // קריאת הערים והקטגוריות ממאגרי המידע
  useEffect(() => {
    // קבלת רשימת הערים
    getCity()
      .then((y) => {
    ...
    // קריאת הדירה לפי ה-ID
    getApartmentById(it)
      .then((y) => {
        const apartment = y.data;
        setThisApartment(apartment);
        setApartmentName(apartment.apartamaentName || '');
        setDescripting(apartment.descripting || '');
        setPicture(apartment.picture || '');
        setCategory(apartment.category || '');
        setCity(apartment.city || '');
        setAddress(apartment.address || '');
        setBedsNumber(apartment.bedsNumber || '');
        setMoreThings(apartment.moreThings || '');
...
console.log("apartmentData",apartmentData)
    // קריאה לעדכון הדירה
    updateApartment(it, localStorage.getItem('_id'),apartmentData)
      .then(e => {
        console.log('Apartment updated successfully:', e);
        Swal.fire({
          icon: "success",
          title: "הדירה עודכנה בהצלחה",
          timer: 2000
      });
      navigate('/Advertiser')
      ...
      .catch(error => {
        ...
        Swal.fire({
          icon: "error",
          text:  "שגיאה בעדכון הדירה",
        });
      });
  };

  if (!thisApartment) {
    return <div>טוען דירה...</div>;
  }
